...
export default function SearchAddCity({ onAdded }: { onAdded: () => void }) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      <form onSubmit={addCity} className="relative w-full">
        {/* Search container with glassmorphic effect */}
        <div
          className="w-full relative flex items-center gap-3 bg-[#1a1a1a] border border-white/10 
               rounded-2xl p-2 shadow-lg focus-within:border-white/30 focus-within:shadow-xl
        ...
        >
          ...
          {/* Input field - allow it to shrink with min-w-0 */}
          <input
            className="flex-1 min-w-0 bg-transparent text-white placeholder-gray-500 
                 text-sm sm:text-base outline-none px-2 py-2.5
                 focus:ring-0"
          ...
          {/* Add button - fixed sizing so it doesn't overflow */}
          <button
            type="submit"
            disabled={loading || !q.trim()}
            className="flex-none px-3 sm:px-6 py-2.5 rounded-xl font-semibold
                 ...
                 max-w-[120px] sm:max-w-40 truncate"
          >
            {loading ? (
              <>
